...
fn solve1(lines: &Vec<String>) -> String {
    ...
    let mut reg_a = lines[0]
        ...
        .parse::<usize>()
        .unwrap();
    let mut reg_b = lines[1]
        .split(": ")
        .last()
        .unwrap()
        .parse::<usize>()
        .unwrap();
    let mut reg_c = lines[2]
        .split(": ")
        .last()
        .unwrap()
        .parse::<usize>()
        .unwrap();
    let prog: Vec<usize> = lines[4]
        .split(": ")
        .last()
        .unwrap()
        .split(",")
...
fn solve2(lines: &Vec<String>) -> i32 {
    ...
    let mut oper: usize;
    let mut combo: usize = 0;
    let mut reg_a_0: usize = 2_usize.pow(44);
        ...
        'run: loop {
            ...
            match instr {
                ...
                5 => {
                    ...
                    if out.len() > prog.len() || out.last().unwrap() != &prog[out.len() - 1] {
                        //println!("{:?}*{:?}*",prog,out);
                        break 'run;
                    }
                }
