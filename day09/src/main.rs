    ...
    while r_pointer >= l_pointer {
        if l_pointer % 2 == 0 {
            ...
            while free_space > 0 && r_pointer > l_pointer {
                ...
                result += id * blocks_to_move * (blocks_to_move - 1) / 2
                    + id * d_pointer * blocks_to_move;
...
fn solve2(lines: &Vec<String>) -> i32 {
    ...
    let mut d_pointer = 0;
    let mut file_size;
    for (i, d_entry) in dmap.iter().enumerate() {
        if i % 2 == 1 {
            gaps.entry(*d_entry).or_insert(vec![]).push(d_pointer);
        } else {
            files.insert(i / 2, (d_pointer, *d_entry));
        }
        d_pointer += d_entry;
    }
